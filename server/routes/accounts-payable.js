...
router.get('/', authMiddleware, (req, res) => {
  ...
  let sql = `SELECT ap.*, s.name as supplier_name, p.name as project_name FROM accounts_payable ap LEFT JOIN suppliers s ON ap.supplier_id = s.id LEFT JOIN projects p ON ap.project_id = p.id WHERE 1=1`;
  const params = [];
...
router.post('/', authMiddleware, validateBody(schemas.accountsPayable), (req, res) => {
  ...
  const db = getDb();
  const id = uuidv4();
  db.run(`INSERT INTO accounts_payable (id, supplier_id, project_id, title, amount, due_date, invoice_no, description, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, supplier_id, project_id || null, title, toCents(amount) ?? 0, due_date || null, invoice_no || '', description || '', req.user.id]);
