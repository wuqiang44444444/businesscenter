...
router.get('/', authMiddleware, (req, res) => {
  ...
  let sql = `SELECT p.*, c.name as customer_name FROM projects p LEFT JOIN customers c ON p.customer_id = c.id WHERE 1=1`;
  const params = [];
...
router.post('/', authMiddleware, validateBody(schemas.project), (req, res) => {
  ...
  const db = getDb();
  const id = uuidv4();
  db.run(`INSERT INTO projects (id, name, customer_id, status, start_date, end_date, description, manager, budget, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, name, customer_id || null, status || 'active', start_date || null, end_date || null, description || '', manager || '', toCents(budget) ?? 0, req.user.id]);
